...
*Longer-horizon: domain-specific fine-tuning.* TTB reviews on the order of 150,000 label applications per year (per the compliance division), and that volume has presumably grown over time — but even at a fraction of current scale, the outcomes of those reviews (approved, rejected, and the specific findings that drove each decision) likely constitute a substantial labeled training corpus. A small vision model fine-tuned on that data would almost certainly outperform a general-purpose 7B model on this specific task — better accuracy on stylized label text, faster inference, lower per-query cost, and no dependency on an external API. The `LabelReader` abstraction is the natural insertion point. The practical obstacles are not technical: they involve data governance (whether historical application materials can be used for training), model auditability requirements under applicable federal AI policy, and whether past decisions are reliable enough ground truth given that some rejections may have been erroneous. These are solvable problems, but they require policy and legal groundwork that is outside the scope of a prototype and is actively evolving at the federal level.
